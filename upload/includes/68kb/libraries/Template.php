	...
	*/
	public function build($view = '', $data = array(), $return = FALSE)
	{
		...
		else 
		{
			...
			/*
			if ($this->_ci->settings->get_setting('site_cache_time') > 0 && ! isset($template['no_cache']))
			{
				$this->_ci->output->cache($this->_ci->settings->get_setting('site_cache_time'));
			}
			*/
		}
    	...
    	$this->_body = $this->_load_view($view, $data);
...
/* Location: ./includes/iclassengine/libraries/Template.php */ 