...
class Cache
{
	...
	public function __construct()
	{
		...
		$this->path = $this->_ci->config->item('cache_path');
		$this->default_expires = $this->_ci->settings->get_setting('site_cache_time');
		if ( ! is_dir($this->path))
		{
	...
	public function get_dependencies() { return $this->dependencies; }
	
	/**
	 * Helper function to get the cache creation date
	 */
	public function get_created($created) { return $this->created; }
	...
	public function delete_all($dirname = '')
	{
		...
		if (file_exists($this->path.$dirname)) delete_files($this->path.$dirname);
...
/* Location: ./upload/includes/iclassengine/libraries/Cache.php */ 